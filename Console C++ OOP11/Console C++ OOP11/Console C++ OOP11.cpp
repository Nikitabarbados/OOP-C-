...
//
//		// записуємо кожний рядок: спочатку довжину, потім сам текст
//		for (int i = 0; i < size; i++) {
//			int len = arr[i].size();
//			out.write((char*)&len, sizeof(len));          // довжина рядка
//			out.write(arr[i].c_str(), len);               // сам рядок
//		}
//		out.close();
//
//		// === Читання з файлу ===
...
//
//		int count;
//		in.read((char*)&count, sizeof(count)); // читаємо кількість рядків
//		string* newArr = new string[count];
//
//		for (int i = 0; i < count; i++) {
//			int len;
//			in.read((char*)&len, sizeof(len));   // читаємо довжину рядка
//			char* buffer = new char[len + 1];    // виділяємо буфер
//			in.read(buffer, len);                // читаємо символи
//			buffer[len] = '\0';                  // додаємо \0
//			newArr[i] = buffer;                  // зберігаємо у string
//			delete[] buffer;
//		}
//		in.close();
//
//		// === Вивід на екран ===
//		cout << "Зчитані з файлу рядки:\n";
//		for (int i = 0; i < count; i++) {
//			cout << newArr[i] << endl;
//		}
//
...
//
//	// створюємо вхідний файловий потік і приєднуємо до нього файл, який відкривається на читання у текстовому режимі (файл повинен існувати)
//	ifstream in; // У ПРОГРАМУ - З ФАЙЛУ
//	in.open("C:\\1\\source.txt", ios::in);
//	if (!in) // якщо відкриття файлу завершилося невдачею - виходимо
//	{
//		cout << "Помилка StringByStringExample!\n";
...
// ----------------------------------------------------------------
//class Student {
//public: // у файл буде записуватися лише стан об'єкта (а не поведінка)
//	string name = "Alex"; // 40
//	string surname = "Petrenko"; // 40
//	int* rates = new int[5] {12, 10, 11, 7, 9}; // 8 (20 байтів корисних даних)
//	unsigned int rates_count = 5; // 4
//public:
//	// тут можуть бути геттери та сеттери для полів, конструктори, деструктор тощо
//	void Print() const
//	{
//		cout << "Name: " << name << endl;
...
//	SetConsoleCP(1251);
//
//	string path = "C:/Users/Alex/Desktop/student.txt";
//	/*
//	Student s;
//	s.Print();
//
//	ofstream out(path, ios::binary | ios::out | ios::trunc); // створюємо вихідний файловий потік і приєднуємо до нього файл, який відкривається на запис
//	if (!out.is_open()) // якщо відкриття файлу завершилося невдачею - виходимо
//	{
//		cout << "Помилка WRITE\n";
//		return -1;
...
//	cout << sizeof(int) << endl;
//	cout << sizeof(Student) << endl; // 96
//	*/
//
//	Student another;
//	another.name = "";
//	another.surname = "";
//	another.rates = nullptr;
//	another.rates_count = 0;
//
//	ifstream in(path, ios::binary | ios::in); // створюємо вхідний файловий потік і приєднуємо до нього файл, який відкривається на читання у текстовому режимі
//	if (!in.is_open()) // якщо відкриття файлу завершилося невдачею - виходимо
//	{
//		cout << "Помилка READ\n";
//		return -1;
...
//
//	// ХАЛЕПА!!!
//	// у файлі збережена певна адреса, по якій КОЛИСЬ знаходився динамічний масив 
//	// якщо після запису у файл, завершити процес (закрити програму), то пам'ять буде автоматично вивільнена ОС!!!
//	// висновок: якщо в класі є поле-покажчик, то записати його в файл буде складніше, одного виклику функциіі райт не вистачить,
//	// треба буде зробити глибоке копіювання об'єкта, всі поля записати ПООКРЕМО
//	in.read((char*)&another, sizeof(Student));
//
//	another.Print();
//
//	Sleep(INFINITE);
...
//	/*
//	Student s;
//	s.rates.push_back(11);
//	s.rates.push_back(12);
//	s.rates.push_back(10);
//	s.rates.push_back(9);
//	s.rates.push_back(8);
//
//	s.Print();
...
//
//	// ХАЛЕПА2
//	// навіть якщо покластися на готовий контейнер типу вектора, то все одно, в нього є своя внутрішня реалізація, ЯКА ПОБУДОВАНА НА ПОКАЖЧИКАХ!!!
//	in.read((char*)&another, sizeof(Student));
//
//	another.Print();
//
...
//	int* rates = new int[5] {12, 10, 11, 7, 9};
//	unsigned int rates_count = 5;
//public:
//	// тут можуть бути геттери та сеттери для полів, конструктори, деструктор тощо
//	void Print() const
//	{
...
//	}
//
//	// рядка нижче НЕ ВИСТАЧИТЬ !!!
//	// out.write((char*)&s, sizeof(s));
//	// на практиці, все буде ЩЕ складніше, тому що поля закриті, і доступ до них буде ЧЕРЕЗ ГЕТТЕРИ !!!
//	int name_length = s.name.length(); // 4
//	int surname_length = s.surname.length(); // 8
//	out.write((char*)&name_length, sizeof(int));
//	out.write((char*)s.name.c_str(), s.name.length()); // name
//	out.write((char*)&surname_length, sizeof(int));
//	out.write((char*)s.surname.c_str(), s.surname.length()); // surname
//	out.write((char*)&s.rates_count, sizeof(int)); // rates_count
//	out.write((char*)s.rates, _msize(s.rates)); // array data
//
//	out.close();
//	cout << "WRITE OK!" << endl;
//	*/
...
//	in.read((char*)&name_length, sizeof(int));
//	name = new char[name_length + 1];
//	in.read((char*)name, name_length); // name
//	another.name.assign(name, name_length);
//
//	in.read((char*)&surname_length, sizeof(int));
//	surname = new char[surname_length + 1];
//	in.read((char*)surname, surname_length); // surname
//	another.surname.assign(surname, surname_length);
//	in.read((char*)&another.rates_count, sizeof(int)); // rates_count
//	another.rates = new int[another.rates_count];
//	in.read((char*)another.rates, sizeof(int) * another.rates_count); // array data
//
//	another.Print();
//	Sleep(INFINITE);
//}
